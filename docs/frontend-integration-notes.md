...
`http://localhost:3000`). В Docker backend слушает порт **3000**.
...
- ⚠️ Если бэкенд работает с локальным MinIO, ссылка ведёт на хост `minio:9000`,
  который браузер по умолчанию не резолвит. Для локальной разработки нужно
  добавить `127.0.0.1 minio` в hosts-файл (см. `docs/s3-integration.md`). На
  внешнем S3 (публичный домен) проблемы нет.
