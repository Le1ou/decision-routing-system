...
previousExecutor?, delegatedByEmployee?, department?, workType?`.

- `author`, `executor`, `previousExecutor`, `delegatedByEmployee`, `department`,
  `workType` — **вложенные объекты** (можно не догружать из справочников).
  `assignedComplexity` может быть пустым — тогда берите `workType.complexity`.
...
1. **`complete` только для назначенного исполнителя** (см. п.6) — кнопки
   «Завершить»/«Взять в работу» руководителю не показывать.
2. **Presigned-ссылки вложений временные** и в локальном MinIO требуют записи в
   hosts (п.7).
3. **Нет предпросмотра приоритета** на сервере (п.8).
4. **Basic Auth на каждом запросе** — нет refresh/logout на сервере; «выход» =
   забыть учётные данные на клиенте.
5. **Видимость/действия — источник истины на сервере.** Не воспроизводите матрицу
...
1. **Блок «Кто делегировал» поддержан контрактом.** Карточка заявки возвращает
   вложенные объекты `delegatedByEmployee` и `previousExecutor`, поэтому frontend
   показывает сотрудника независимо от ограничений `GET /employees`.
...
### Решённые вопросы backend

1. **Виды работ при создании заявки.** `GET /work-types` доступен всем
   авторизованным пользователям без ограничения собственным отделом и
   поддерживает фильтр `departmentId`. Ограничение по отделу сохраняется для
   операций управления справочником.
