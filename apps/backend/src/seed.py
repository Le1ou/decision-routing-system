...
def seed_database(db_operator) -> None:
    ...
    with db_operator.pool.connection() as conn:
        ...
        dep_ids = {}
        # value = коэффициент важности отдела (k_отдела), диапазон [0.5, 1.25]:
        #   важные отделы (>1) сильнее поднимают приоритет, ближе к дедлайну — до critical.
        # deadline_notification = доля ОСТАВШЕГОСЯ времени, при которой шлём уведомление
        # о приближении дедлайна (≈ «осталось ~25%»). Небольшой разброс вокруг 0.25.
        for dep_key, name, group, value, same_dep, delay, notif in (
            ("it",        "ИТ-отдел",                        "Основной",        1.15, False, 30,  0.25),
            ("oge",       "Отдел главного энергетика (ОГЭ)",  "Основной",        0.8,  True,  60,  0.30),
            ("prod",      "Производственный отдел",           "Основной",        1.25, False, 30,  0.20),
            ("okk",       "Отдел контроля качества (ОКК)",    "Основной",        1.0,  True,  45,  0.30),
            ("ogm",       "Отдел главного механика (ОГМ)",    "Основной",        1.1,  True,  60,  0.25),
            ("warehouse", "Складской отдел",                  "Вспомогательный", 0.5,  False, 120, 0.35),
            ("supply",    "Отдел снабжения",                  "Вспомогательный", 0.65, False, 120, 0.35),
        ):
            row = conn.execute(
                """
